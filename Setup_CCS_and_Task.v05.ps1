...
try {
    ...
    if (-not (Test-Path $TempLogDir)) {
        try {
            ...
            Write-SetupLog -Message "Created log directory: ${TempLogDir} with SYSTEM and Administrators permissions" -EventId 1054
        } catch {
            Write-SetupLog -Message "Failed to create log directory ${TempLogDir}: $($_.Exception.Message)" -Level "ERROR" -EventId 1055
            throw
        }
    }
    ...
    if (-not $ExportScriptPath) {
        Write-SetupLog -Message "Prompting for export script path" -EventId 1056
        $ExportScriptPath = Read-Host -Prompt "Enter the full path to the export script (e.g., C:\Scripts\Export_Cert_CCS_Secure.ps1)"
        if (-not $ExportScriptPath) {
            Write-SetupLog -Message "Export script path is required" -Level "ERROR" -EventId 1057
            throw "Export script path not provided"
        }
        Write-SetupLog -Message "Export script path set to: ${ExportScriptPath}" -EventId 1058
    }

    # Prompt for PfxPassword if not provided
    if (-not $PfxPassword) {
        Write-SetupLog -Message "Prompting for PFX password" -EventId 1059
        $PfxPassword = Read-Host -AsSecureString -Prompt "Enter the PFX password"
        if (-not $PfxPassword) {
            Write-SetupLog -Message "PFX password is required" -Level "ERROR" -EventId 1060
            throw "PFX password not provided"
        }
        Write-SetupLog -Message "Received PFX password" -EventId 1061
    }

    # Prompt for NupkgPath if CredentialManager module is missing
    if (-not (Get-Module -ListAvailable -Name CredentialManager)) {
        Write-SetupLog -Message "Prompting for CredentialManager .nupkg path" -EventId 1062
        ...
        if (-not $NupkgPath -or -not (Test-Path $NupkgPath)) {
            Write-SetupLog -Message "CredentialManager .nupkg file not found at ${NupkgPath}" -Level "ERROR" -EventId 1063
            throw "CredentialManager .nupkg file not found"
        }
        Write-SetupLog -Message "Received NupkgPath: ${NupkgPath}" -EventId 1064
        try {
            ...
            Write-SetupLog -Message "Installed CredentialManager module to ${modulePath}" -EventId 1065
        } catch {
            Write-SetupLog -Message "Failed to install CredentialManager module from ${NupkgPath}: $($_.Exception.Message)" -Level "ERROR" -EventId 1066
            throw
        }
    }
    ...
    try {
        ...
        Write-SetupLog -Message "Imported CredentialManager module" -EventId 1067
    } catch {
        Write-SetupLog -Message "Failed to import CredentialManager module: $($_.Exception.Message)" -Level "ERROR" -EventId 1068
        throw
    }
    ...
    try {
        ...
        Write-SetupLog -Message "Stored PFX password in Credential Manager (masked): ${maskedPassword}" -EventId 1069
    } catch {
        Write-SetupLog -Message "Failed to store PFX password in Credential Manager: $($_.Exception.Message)" -Level "ERROR" -EventId 1070
        throw
    }
    ...
    try {
        ...
        Write-SetupLog -Message "CCS registry key found: Enabled=$($registryValues.Enabled), CertStoreLocation=$($registryValues.CertStoreLocation)" -EventId 1071
        if ($registryValues.Enabled -ne 1 -or $registryValues.CertStoreLocation -ne $CcsPhysicalPath) {
            Write-SetupLog -Message "CCS registry key values are incorrect (Enabled=$($registryValues.Enabled), CertStoreLocation=$($registryValues.CertStoreLocation)). Expected Enabled=1 and CertStoreLocation=$CcsPhysicalPath" -Level "ERROR" -EventId 1072
            throw "CCS registry configuration is invalid"
        }
    } catch {
        Write-SetupLog -Message "CCS registry key not found at ${registryPath}. Creating key." -Level "WARNING" -EventId 1073
        try {
            ...
            Write-SetupLog -Message "Created CCS registry key: Enabled=1, CertStoreLocation=$CcsPhysicalPath" -EventId 1074
        } catch {
            Write-SetupLog -Message "Failed to configure CCS registry at ${registryPath}: $($_.Exception.Message)" -Level "ERROR" -EventId 1075
            throw
        }
    }
    ...
    try {
        if (-not (Test-Path $CcsPhysicalPath)) {
            Write-SetupLog -Message "CCS path ${CcsPhysicalPath} is not accessible" -Level "ERROR" -EventId 1076
            throw "CCS path inaccessible"
        }
        Write-SetupLog -Message "CCS path ${CcsPhysicalPath} is accessible" -EventId 1077
    } catch {
        Write-SetupLog -Message "Error accessing CCS path ${CcsPhysicalPath}: $($_.Exception.Message)" -Level "ERROR" -EventId 1078
        throw
    }
...
"@
        ...
        try {
            ...
            Write-SetupLog -Message "Registered new scheduled task: ${TaskName}" -EventId 1079
        } catch {
            Write-SetupLog -Message "Updating existing scheduled task: ${TaskName}" -EventId 1080
            $folder.RegisterTaskDefinition($TaskName, $taskDefinition, 4, $null, $null, 3) | Out-Null
        }
    } catch {
        Write-SetupLog -Message "Failed to register or update scheduled task ${TaskName}: $($_.Exception.Message)" -Level "ERROR" -EventId 1081
        throw
    }
    ...
    try {
        ...
        Write-SetupLog -Message "Verified scheduled task '${TaskName}' exists" -EventId 1082
        Log-TaskSchedulerEvents -TaskName $TaskName
    } catch {
        Write-SetupLog -Message "Failed to verify scheduled task '${TaskName}': $($_.Exception.Message)" -Level "ERROR" -EventId 1083
        throw
    }
    ...
    if (-not (Test-SystemWritePermission -Path $writableDir)) {
        Write-SetupLog -Message "SYSTEM write permission test to ${writableDir} failed" -Level "ERROR" -EventId 1084
    ...
    Write-SetupLog -Message "Setup completed successfully" -EventId 1085
} catch {
    Write-SetupLog -Message "Setup failed: $($_.Exception.Message)" -Level "ERROR" -EventId 1086
    throw
}
