...
try {
    ...
    if (`$credential) {
        for (`$i = 0; `$i -lt 3; `$i++) {
            try {
                "Credential found: UserName=`$(`$credential.UserName), Password=`$(`$credential.GetNetworkCredential().Password)" | Out-File -FilePath '${testLog}' -Append -ErrorAction Stop
                break
            } catch {
                Start-Sleep -Milliseconds 500
                if (`$i -eq 2) { throw `$_.Exception }
...
"@
        ...
        $verifyCommand = "powershell.exe -NoProfile -ExecutionPolicy Bypass -File `"${verifyScript}`""
        $action = New-ScheduledTaskAction -Execute "powershell.exe" -Argument $verifyCommand
        $principal = New-ScheduledTaskPrincipal -UserId "NT AUTHORITY\SYSTEM" -LogonType ServiceAccount -RunLevel Highest
...
try {
    # Validate PfxPassword
    try {
        $testPassword = Convert-SecureStringToPlainText -SecureString $PfxPassword
        if (-not $testPassword) {
            Write-SetupLog -Message "Provided PfxPassword is empty or invalid" -Level "ERROR" -EventId 1047
            throw "Invalid PfxPassword"
        }
        Write-SetupLog -Message "PfxPassword validated successfully" -EventId 1048
    } catch {
        Write-SetupLog -Message "Failed to validate PfxPassword: $($_.Exception.Message)" -Level "ERROR" -EventId 1049
        throw
    }
    ...
    if ($taskService.Status -ne 'Running') {
        Write-SetupLog -Message "Task Scheduler service is not running. Current status: $($taskService.Status)" -Level "ERROR" -EventId 1050
        throw "Task Scheduler service is not running"
    }
    Write-SetupLog -Message "Task Scheduler service is running" -EventId 1051

    # Check PowerShell execution policy
    $executionPolicy = Get-ExecutionPolicy -Scope LocalMachine
    if ($executionPolicy -eq 'Restricted' -or $executionPolicy -eq 'AllSigned') {
        Write-SetupLog -Message "PowerShell execution policy is ${executionPolicy}. This may prevent tasks from running. Consider setting to RemoteSigned or Bypass." -Level "WARNING" -EventId 1052
    }
    Write-SetupLog -Message "PowerShell execution policy: ${executionPolicy}" -EventId 1053

    # Log module paths for SYSTEM context
    $modulePath = $env:PSModulePath
    Write-SetupLog -Message "PowerShell module path: ${modulePath}" -EventId 1054

    # Log security policy information
    try {
        $appLockerPolicy = Get-AppLockerPolicy -Effective -Local -ErrorAction Stop
        Write-SetupLog -Message "AppLocker policy: $($appLockerPolicy | Out-String)" -EventId 1055
    } catch {
        Write-SetupLog -Message "Failed to retrieve AppLocker policy: $($_.Exception.Message)" -Level "WARNING" -EventId 1056
    }
    try {
        $groupPolicy = gpresult /R /SCOPE COMPUTER
        Write-SetupLog -Message "Group Policy (Computer scope): $($groupPolicy -join ', ')" -EventId 1057
    } catch {
        Write-SetupLog -Message "Failed to retrieve Group Policy: $($_.Exception.Message)" -Level "WARNING" -EventId 1058
    }

    # Prompt for ExportScriptPath if not provided
    if (-not $ExportScriptPath) {
        Write-SetupLog -Message "ExportScriptPath parameter not provided. Prompting for input." -Level "WARNING" -EventId 1059
        if (Test-InteractiveSession) {
            $ExportScriptPath = Read-Host -Prompt "Enter the full path to the export script (e.g., C:\Scripts\Export_Cert_CCS_Secure.ps1)"
            if (-not $ExportScriptPath) {
                Write-SetupLog -Message "No export script path provided. A valid path is required." -Level "ERROR" -EventId 1060
                throw "Export script path not provided"
            }
        } else {
            Write-SetupLog -Message "ExportScriptPath parameter not provided and non-interactive session detected. A valid path is required." -Level "ERROR" -EventId 1061
    ...
    if (-not (Test-Path $TempLogDir)) {
        try {
            New-Item -Path $TempLogDir -ItemType Directory -Force | Out-Null
            icacls $TempLogDir /grant "NT AUTHORITY\SYSTEM:(OI)(CI)(F)" /grant "Administrators:(OI)(CI)(F)" | Out-Null
            Write-SetupLog -Message "Created temporary log directory: ${TempLogDir} with SYSTEM and Administrators permissions" -EventId 1062
        } catch {
            Write-SetupLog -Message "Failed to create temporary log directory ${TempLogDir}: $($_.Exception.Message)" -Level "ERROR" -EventId 1063
            throw
        }
    }
    if (-not (Test-SystemWritePermission -Path $TempLogDir)) {
        Write-SetupLog -Message "SYSTEM account cannot write to ${TempLogDir}. Attempting to find a SYSTEM-writable directory." -Level "WARNING" -EventId 1064
        $TempLogDir = Find-SystemWritableDirectory
        Write-SetupLog -Message "Using SYSTEM-writable directory: ${TempLogDir}" -EventId 1065
    }
    $acl = icacls $TempLogDir
    Write-SetupLog -Message "Permissions for ${TempLogDir}: $($acl -join ', ')" -EventId 1066
    ...
    if ($osVersion -notlike "*Windows Server 2019*" -and $osVersion -notlike "*Windows Server 2022*") {
        Write-SetupLog -Message "Unsupported operating system: ${osVersion}. This script is designed for Windows Server 2019 or 2022." -Level "ERROR" -EventId 1067
        throw "Unsupported operating system"
    }
    Write-SetupLog -Message "Running on ${osVersion}" -EventId 1068
    ...
    if (-not $scriptingFeature.Installed) {
        Write-SetupLog -Message "Installing Web-Scripting-Tools feature" -EventId 1069
        Install-WindowsFeature -Name Web-Scripting-Tools -ErrorAction Stop
        Write-SetupLog -Message "Web-Scripting-Tools feature installed" -EventId 1070
    } else {
        Write-SetupLog -Message "Web-Scripting-Tools feature is already installed" -EventId 1071
    }
    $ccsFeature = Get-WindowsFeature -Name Web-CertProvider
    if (-not $ccsFeature.Installed) {
        Write-SetupLog -Message "Installing Web-CertProvider feature" -EventId 1072
        Install-WindowsFeature -Name Web-CertProvider -ErrorAction Stop
        Write-SetupLog -Message "Web-CertProvider feature installed" -EventId 1073
    } else {
        Write-SetupLog -Message "Web-CertProvider feature is already installed" -EventId 1074
    }

    # Check for CredentialManager module
    if (-not (Get-Module -ListAvailable -Name CredentialManager)) {
        Write-SetupLog -Message "CredentialManager module not found." -Level "WARNING" -EventId 1075
        if (-not $NupkgPath -or -not (Test-Path $NupkgPath)) {
            if (Test-InteractiveSession) {
                Write-SetupLog -Message "Prompting for CredentialManager .nupkg file location." -EventId 1076
                ...
                if (-not $NupkgPath -or -not (Test-Path $NupkgPath)) {
                    Write-SetupLog -Message "Invalid or missing .nupkg file path provided: ${NupkgPath}. To obtain the .nupkg file, run 'Save-Module -Name CredentialManager -Path C:\Temp\Modules' on a machine with internet access and transfer the file to this server." -Level "ERROR" -EventId 1077
                    throw "CredentialManager module not installed and no valid .nupkg file provided"
                }
            } else {
                Write-SetupLog -Message "CredentialManager module not found and no valid .nupkg file path provided in non-interactive session. Please provide a valid CredentialManager .nupkg file path (e.g., C:\Packages\CredentialManager.2.0.nupkg)." -Level "ERROR" -EventId 1078
                Write-SetupLog -Message "To obtain the .nupkg file, run 'Save-Module -Name CredentialManager -Path C:\Temp\Modules' on a machine with internet access and transfer the file to this server." -Level "ERROR" -EventId 1079
        ...
        Write-SetupLog -Message "CredentialManager module is already installed" -EventId 1080
    }
    Import-Module CredentialManager -ErrorAction Stop
    Write-SetupLog -Message "Imported CredentialManager module" -EventId 1081
    ...
    try {
        ...
        Write-SetupLog -Message "CCS registry key found: Enabled=$($registryValues.Enabled), PhysicalPath=$($registryValues.CertStoreLocation)" -EventId 1082
        if ($registryValues.Enabled -ne 1 -or $registryValues.CertStoreLocation -ne $CcsPhysicalPath) {
            Write-SetupLog -Message "CCS registry key values are incorrect (Enabled=$($registryValues.Enabled), PhysicalPath=$($registryValues.CertStoreLocation)). Expected Enabled=1 and PhysicalPath=$CcsPhysicalPath. Will not modify existing settings." -Level "ERROR" -EventId 1083
        ...
        Write-SetupLog -Message "Created CCS registry key: Enabled=1, PhysicalPath=$CcsPhysicalPath" -EventId 1085
    }
    ...
    try {
        ...
        if (-not $testPath) {
            Write-SetupLog -Message "CCS path ${CcsPhysicalPath} is not accessible. Ensure the share is configured to allow access by the computer account." -Level "ERROR" -EventId 1086
            throw "CCS path inaccessible"
        }
        Write-SetupLog -Message "CCS path ${CcsPhysicalPath} is accessible" -EventId 1087
    } catch {
        Write-SetupLog -Message "Error accessing CCS path ${CcsPhysicalPath}: $($_.Exception.Message)" -Level "ERROR" -EventId 1088
        throw
    }
...
"@
    try {
        ...
        Write-SetupLog -Message "Set SYSTEM permissions on ${storeScript}" -EventId 1089
        $storeCommand = "powershell.exe -NoProfile -ExecutionPolicy Bypass -File `"${storeScript}`""
        $action = New-ScheduledTaskAction -Execute "powershell.exe" -Argument $storeCommand
        $principal = New-ScheduledTaskPrincipal -UserId "NT AUTHORITY\SYSTEM" -LogonType ServiceAccount -RunLevel Highest
        ...
        Write-SetupLog -Message "Temporary task ${tempTaskName} created to store credential as SYSTEM" -EventId 1090
    } catch {
        Write-SetupLog -Message "Failed to register temporary task ${tempTaskName}: $($_.Exception.Message)" -Level "ERROR" -EventId 1091
        throw
    }
    ...
    try {
        ...
        Write-SetupLog -Message "Started temporary task ${tempTaskName}" -EventId 1092
        Start-Sleep -Seconds 60
        ...
        if (Test-Path $tempLogPath) {
            ...
            if ($taskLog -match "Credential stored successfully") {
                Write-SetupLog -Message "Temporary task ${tempTaskName} executed successfully. Log content: ${taskLog}" -EventId 1093
            } else {
                Write-SetupLog -Message "Temporary task ${tempTaskName} failed. Log content: ${taskLog}" -Level "ERROR" -EventId 1094
                throw "Temporary task execution failed"
            }
        } else {
            Write-SetupLog -Message "Temporary task log file not found at ${tempLogPath}. Credential storage failed." -Level "ERROR" -EventId 1095
            throw "Credential storage failed"
        }
    } catch {
        Write-SetupLog -Message "Error running temporary task ${tempTaskName}: $($_.Exception.Message)" -Level "ERROR" -EventId 1096
        throw
    } finally {
        ...
        Unregister-ScheduledTask -TaskName $tempTaskName -Confirm:$false -ErrorAction SilentlyContinue
        Write-SetupLog -Message "Temporary task ${tempTaskName} and files removed" -EventId 1097
    }
    ...
    for ($i = 0; $i -lt $retryCount; $i++) {
        try {
            ...
            if ($credentialVerified) {
                ...
                Write-SetupLog -Message "Attempt $($i + 1) to verify credential failed. Retrying after ${retryDelay} seconds." -Level "WARNING" -EventId 1098
                if ($i -lt $retryCount - 1) {
            ...
            Write-SetupLog -Message "Attempt $($i + 1) to verify credential failed: $($_.Exception.Message)" -Level "WARNING" -EventId 1099
            if ($i -lt $retryCount - 1) {
                Start-Sleep -Seconds $retryDelay
                continue
            }
    ...
    if (-not $credentialVerified) {
        Write-SetupLog -Message "Failed to verify stored credential for PFXCertPassword after $retryCount attempts. Credential is not found." -Level "ERROR" -EventId 1100
        Write-SetupLog -Message "Please manually store the credential as SYSTEM using: New-StoredCredential -Target 'PFXCertPassword' -UserName 'PFXUser' -Password '<YourPassword>' -Persist LocalMachine" -Level "ERROR" -EventId 1101
        throw "Credential storage failed"
    }

    # Verify export script exists
    if (-not (Test-Path $ExportScriptPath)) {
        Write-SetupLog -Message "Export script not found at ${ExportScriptPath}" -Level "ERROR" -EventId 1102
...
"@
        ...
        try {
            ...
            Write-SetupLog -Message "Registered new scheduled task: ${TaskName}" -EventId 1103
        } catch {
            Write-SetupLog -Message "Updating existing scheduled task: ${TaskName}" -EventId 1104
            $folder.RegisterTaskDefinition($TaskName, $taskDefinition, 4, $null, $null, 3) | Out-Null
        }
    } catch {
        Write-SetupLog -Message "Failed to register or update scheduled task ${TaskName}: $($_.Exception.Message)" -Level "ERROR" -EventId 1105
        throw
    }

    Write-SetupLog -Message "Setup completed successfully" -EventId 1106
} catch {
    Write-SetupLog -Message "Error during setup: $($_.Exception.Message)" -Level "ERROR" -EventId 1107
    throw
}
