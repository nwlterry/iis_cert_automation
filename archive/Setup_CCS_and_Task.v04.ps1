...
function Start-IISManager {
    try {
        ...
        if (Test-InteractiveSession) {
            Write-SetupLog -Message "Waiting for user to complete IIS Manager configuration. Press Enter when done." -EventId 1036
            Read-Host | Out-Null
        } else {
...
try {
    ...
    if (-not (Test-Path $TempLogDir)) {
        try {
            ...
            Write-SetupLog -Message "Created log directory: ${TempLogDir} with SYSTEM and Administrators permissions" -EventId 1052
        } catch {
            Write-SetupLog -Message "Failed to create log directory ${TempLogDir}: $($_.Exception.Message)" -Level "ERROR" -EventId 1053
            throw
        }
    }

    # Prompt for ExportScriptPath if not provided in interactive session
    if (-not $ExportScriptPath -and (Test-InteractiveSession)) {
        $ExportScriptPath = Read-Host "Enter the path to the export script (e.g., C:\Scripts\Export_Cert_CCS_Secure.ps1)"
        if (-not $ExportScriptPath) {
            Write-SetupLog -Message "ExportScriptPath is required" -Level "ERROR" -EventId 1054
            throw "ExportScriptPath not provided"
        }
    }

    # Prompt for PfxPassword if not provided in interactive session
    if (-not $PfxPassword -and (Test-InteractiveSession)) {
        $PfxPassword = Read-Host -AsSecureString "Enter the PFX password"
        if (-not $PfxPassword) {
            Write-SetupLog -Message "PFX password is required" -Level "ERROR" -EventId 1055
    ...
    if (-not (Get-Module -ListAvailable -Name CredentialManager)) {
        if (Test-InteractiveSession -and -not $NupkgPath) {
            $NupkgPath = Read-Host "CredentialManager module not found. Enter the path to the CredentialManager .nupkg file (e.g., C:\Temp\credentialmanager.2.0.0.nupkg)"
        }
        if (-not $NupkgPath -or -not (Test-Path $NupkgPath)) {
            Write-SetupLog -Message "CredentialManager .nupkg file not found at ${NupkgPath}" -Level "ERROR" -EventId 1056
        ...
        try {
            ...
            Expand-Archive -Path $NupkgPath -DestinationPath $modulePath -Force -ErrorAction Stop
            Write-SetupLog -Message "Installed CredentialManager module to ${modulePath}" -EventId 1057
        } catch {
            Write-SetupLog -Message "Failed to install CredentialManager module from ${NupkgPath}: $($_.Exception.Message)" -Level "ERROR" -EventId 1058
            throw
        }
    }
    ...
    try {
        ...
        Write-SetupLog -Message "Imported CredentialManager module" -EventId 1059
    } catch {
        Write-SetupLog -Message "Failed to import CredentialManager module: $($_.Exception.Message)" -Level "ERROR" -EventId 1060
        throw
    }

    # Store PFX password in Credential Manager
    try {
        ...
        Write-SetupLog -Message "Stored PFX password in Credential Manager (masked): ${maskedPassword}" -EventId 1061
    } catch {
        Write-SetupLog -Message "Failed to store PFX password in Credential Manager: $($_.Exception.Message)" -Level "ERROR" -EventId 1062
        throw
    }
    ...
    try {
        ...
        Write-SetupLog -Message "CCS registry key found: Enabled=$($registryValues.Enabled), CertStoreLocation=$($registryValues.CertStoreLocation)" -EventId 1063
        if ($registryValues.Enabled -ne 1 -or $registryValues.CertStoreLocation -ne $CcsPhysicalPath) {
            Write-SetupLog -Message "CCS registry key values are incorrect (Enabled=$($registryValues.Enabled), CertStoreLocation=$($registryValues.CertStoreLocation)). Expected Enabled=1 and CertStoreLocation=$CcsPhysicalPath" -Level "ERROR" -EventId 1064
            throw "CCS registry configuration is invalid"
        }
    } catch {
        Write-SetupLog -Message "CCS registry key not found at ${registryPath}. Creating key." -Level "WARNING" -EventId 1065
        try {
            ...
            Write-SetupLog -Message "Created CCS registry key: Enabled=1, CertStoreLocation=$CcsPhysicalPath" -EventId 1066
        } catch {
            Write-SetupLog -Message "Failed to configure CCS registry at ${registryPath}: $($_.Exception.Message)" -Level "ERROR" -EventId 1067
            throw
        }
    }
    ...
    try {
        if (-not (Test-Path $CcsPhysicalPath)) {
            Write-SetupLog -Message "CCS path ${CcsPhysicalPath} is not accessible" -Level "ERROR" -EventId 1068
            throw "CCS path inaccessible"
        }
        Write-SetupLog -Message "CCS path ${CcsPhysicalPath} is accessible" -EventId 1069
    } catch {
        Write-SetupLog -Message "Error accessing CCS path ${CcsPhysicalPath}: $($_.Exception.Message)" -Level "ERROR" -EventId 1070
        throw
    }
...
"@
        ...
        try {
            ...
            Write-SetupLog -Message "Registered new scheduled task: ${TaskName}" -EventId 1071
        } catch {
            Write-SetupLog -Message "Updating existing scheduled task: ${TaskName}" -EventId 1072
            $folder.RegisterTaskDefinition($TaskName, $taskDefinition, 4, $null, $null, 3) | Out-Null
        }
    } catch {
        Write-SetupLog -Message "Failed to register or update scheduled task ${TaskName}: $($_.Exception.Message)" -Level "ERROR" -EventId 1073
        throw
    }
    ...
    try {
        ...
        Write-SetupLog -Message "Verified scheduled task '${TaskName}' exists" -EventId 1074
        Log-TaskSchedulerEvents -TaskName $TaskName
    } catch {
        Write-SetupLog -Message "Failed to verify scheduled task '${TaskName}': $($_.Exception.Message)" -Level "ERROR" -EventId 1075
        throw
    }
    ...
    if (-not (Test-SystemWritePermission -Path $writableDir)) {
        Write-SetupLog -Message "SYSTEM write permission test to ${writableDir} failed" -Level "ERROR" -EventId 1076
    ...
    Write-SetupLog -Message "Setup completed successfully" -EventId 1077
} catch {
    Write-SetupLog -Message "Setup failed: $($_.Exception.Message)" -Level "ERROR" -EventId 1078
    throw
}
