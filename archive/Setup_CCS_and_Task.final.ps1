...
# Function to run IIS Manager and wait for user input or delay in non-interactive mode without reset
function Start-IISManagerNoReset {
    try {
...
"@
    ...
    try {
        ...
        Write-SetupLog -Message "CCS registry key not found at ${registryPath}. Creating key." -Level "WARNING" -EventId 1097
        try {
            ...
            Write-SetupLog -Message "Created CCS registry key: Enabled=1, CertStoreLocation=$CcsPhysicalPath" -EventId 1098
        } catch {
            Write-SetupLog -Message "Failed to configure CCS registry at ${registryPath}: $($_.Exception.Message)" -Level "ERROR" -EventId 1099
            throw
        }
    }

    # Run IIS Manager
    Start-IISManagerNoReset

    # Verify CCS path accessibility
    try {
        if (-not (Test-Path $CcsPhysicalPath)) {
            Write-SetupLog -Message "CCS path ${CcsPhysicalPath} is not accessible" -Level "ERROR" -EventId 1100
            throw "CCS path inaccessible"
        }
        Write-SetupLog -Message "CCS path ${CcsPhysicalPath} is accessible" -EventId 1101
    } catch {
        Write-SetupLog -Message "Error accessing CCS path ${CcsPhysicalPath}: $($_.Exception.Message)" -Level "ERROR" -EventId 1102
        throw
    }
...
"@
    ...
    try {
        ...
        Write-SetupLog -Message "Verified scheduled task '${TaskName}' exists" -EventId 1106
        Log-TaskSchedulerEvents -TaskName $TaskName
    } catch {
        Write-SetupLog -Message "Failed to verify scheduled task '${TaskName}': $($_.Exception.Message)" -Level "ERROR" -EventId 1107
        throw
    }

    # Perform initial certificate export by triggering the scheduled task
    try {
        Write-SetupLog -Message "Starting initial certificate export using scheduled task" -EventId 1111
        Start-ScheduledTask -TaskName $TaskName -ErrorAction Stop
        Write-SetupLog -Message "Waiting for initial export to complete..." -EventId 1112
        ...
        do {
            ...
            if ($taskState -eq "Ready") {
                ...
                if ($taskInfo.LastTaskResult -eq 0) {
                    Write-SetupLog -Message "Initial export completed successfully (LastTaskResult: $($taskInfo.LastTaskResult))" -EventId 1113
                } else {
                    Write-SetupLog -Message "Initial export completed with error (LastTaskResult: $($taskInfo.LastTaskResult))" -Level "WARNING" -EventId 1114
                }
                Log-TaskSchedulerEvents -TaskName $TaskName
                break
            }
            if (((Get-Date) - $startTime).TotalSeconds -gt $timeoutSeconds) {
                Write-SetupLog -Message "Initial export task timed out after $timeoutSeconds seconds" -Level "ERROR" -EventId 1115
                throw "Initial export timeout"
            }
        } while ($true)
    } catch {
        Write-SetupLog -Message "Failed to perform initial certificate export: $($_.Exception.Message)" -Level "ERROR" -EventId 1116
        throw
    }

    # Re-open IIS Manager after initial export
    Start-IISManager
    ...
    if (-not (Test-SystemWritePermission -Path $writableDir)) {
        Write-SetupLog -Message "SYSTEM write permission test to ${writableDir} failed" -Level "ERROR" -EventId 1108
    ...
    Write-SetupLog -Message "Setup completed successfully" -EventId 1109
} catch {
    Write-SetupLog -Message "Setup failed: $($_.Exception.Message)" -Level "ERROR" -EventId 1110
    throw
}
